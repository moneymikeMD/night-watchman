...
| **A measured second consumer is the bar for extracting a shared core.** One real fork of the same code, not an anticipated one, is what turns "keep it here" into "split it". Absent that, the duplication is hypothetical and the extraction is speculative work. | 2026-09-19, NWM-127: `land-branch.sh` splits because `~/code/homelab/scripts/dev/land-branch.sh` measurably exists as a 1402-line fork carrying the same classes of fix independently. Owner confirmed the split; filed as NWM-131. |
