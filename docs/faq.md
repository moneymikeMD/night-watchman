...
`providers/dispatch/` is currently built as a peer implementation rather than a
thin adapter, which means this repo maintains a dispatch mechanism whose failure
modes the platform has since made unnecessary. Three of the open herdr dispatch
...
The honest reading is that layer 3, the provider system, is the least settled of
the three and the dispatch provider is the least settled part of it.
