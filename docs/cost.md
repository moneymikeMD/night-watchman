...
An optional layer (see the README's "Optional layers"): a
...
- `scripts/claude-cost-scan.py` — one way to produce that `--cost`/
  ...
  `subagents/agent-*.jsonl` for Agent-tool turns and
  `subagents/workflows/wf_*/agent-*.jsonl` for Workflow-tool turns),
  dedupes the several JSONL lines a single
...
- `templates/claude-prices.tsv` — the $/million-token price table
  ...
  under `$CLAUDE_PROJECT_DIR`. Because it lives in another repo, the
  search's first candidate is *that* repo's `templates/`, so
  `hooks/script-events-hook.sh` pins `--prices` explicitly rather than
  trusting the order, and a run by hand should do the same.
...
split the same wave by role, so a reviewer can compare what an
...
The weekly cap on `claude-fable-5-1` is separate from every other model's:
Opus, Sonnet and Haiku draw from a different pool. Fable is the default
orchestrator model, so routine dispatch, review triage and landing work
burn the scarcest budget by default, whether or not the wave needed
Fable's quality.
...
Does Opus at high effort hold quality as the orchestrator? No such
comparison has run (NWM-119 is open on it), so this document makes no
recommendation, and the default in `~/.claude/settings.json` does not
change until one has — measure first, then decide.
...
twin — see the README's "Optional layers": the plugin never
...
- `hooks/script-events-hook.sh` — a `SubagentStop` hook (registered in
  ...
  `# script-analytics-extractor-sentinel: v1`.
...
these columns are lifetime figures. `flag` follows a fixed precedence,
evaluated in order:
...
(rework cost / author cost across every script, lifetime with no
`--since`/`--until` given). When `--since` or `--until` IS given, two
...
answered by counting, not by impression.
...
actual typed MCP tool, and picking the pilot, is out of scope here (WO-33
specified this report; WO-29 covers the pilot pattern).
