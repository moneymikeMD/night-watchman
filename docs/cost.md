...
- `templates/claude-prices.tsv` — the $/million-token price table
  ...
  one-time stderr warning, never a hard failure.
...
- `hooks/script-events-hook.sh` — a `SubagentStop` hook (registered in
  ...
  `scripts/script-analytics.py extract --agent-id ...` for just the
  subagent that finished. Idempotent by its own event `key`, fails open on
...
- `scripts/script-analytics.py` — reads local Claude Code JSONL
  ...
  `claude-cost-scan.py`, reusing its timestamp/price helpers and
  `claude-cost.py`'s table/tsv/md renderers rather than duplicating them)
  and appends one JSON object per line to an events file — one row per
  script-author/script-reviewer invocation, lint/selftest call, or
  SendMessage-resume round. `report` summarises that events file by script
