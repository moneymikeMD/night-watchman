...
- `scripts/claude-cost-scan.py` — one way to produce that `--cost`/
  ...
  `subagents/workflows/wf_*/agent-*.jsonl` for Workflow-tool turns — both
  shapes, since NWM-152; missing the second under-counted a wide fan-out by
  a factor of several), dedupes the several JSONL lines a single
