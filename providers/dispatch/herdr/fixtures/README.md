...
| `agent-wait.json` | `herdr agent wait <branch> ...` | real `agent wait` prints no trailing `wait exit 0` line; the capture script's own echo is not part of the response |
...
| `pane-close-help.txt` | `herdr pane close --help` | reference only: `stop` does not call `pane close`, and no live `pane close` response is recorded, so an agent with no workspace_id is refused (exit 2) rather than guessed at |
